    ...
    loop {
      ...
      let question = match packet.first_question() {
        ...
        None => {
          ...
          dns_error!(self.socket, src, response, Rcode::FormErr);
          continue;
        }
      };
      ...
      let res = match self.pdns.lookup_hostname(hostname, qtype) {
        ...
        Err(e) => {
          ...
          dns_error!(self.socket, src, response, Rcode::ServFail);
          continue;
        }
      };
      ...
      match self.socket.send_to(response.finish().as_slice(), src) {
        Ok(n) => debug!("Data sent: {}", n),
        Err(e) => warn!("Error sending response: {}", e),
      }
    }
